...
export function createTenant(full_name: string, phone?: string, id_number?: string, note?: string) {
  const id = uuidv4();
  exec(`INSERT INTO tenants (id, full_name, phone, id_number, note) VALUES (?,?,?,?,?)`,
...
export function getChargeType(id: string) { return query(`SELECT * FROM charge_types WHERE id = ?`, [id])[0]; }
export function listChargeTypes() { return query(`SELECT * FROM charge_types ORDER BY name ASC`); }

// ===== Leases =====
export function getLease(leaseId: string) { return query(`SELECT * FROM leases WHERE id = ?`, [leaseId])[0]; }
...
export function startLeaseAdvanced(cfg: LeaseConfig) {
  ...
  const {
    ...
    baseRent, baseRentCollect='start', deposit=0, durationDays,
  ...
  exec(
    ...
    [id, roomId, leaseType, startDateISO, billing_cycle,
     baseRent, deposit, durationDays ?? null, 0,
     endDateISO ?? null, tenantId, baseRentCollect]
  );
...
export function addRecurringCharge(
  leaseId: string, chargeTypeId: string, unit_price?: number, is_variable?: number, config?: any
) {
  const id = uuidv4();
  let cfg = null;
...
export function listSettledCyclesDesc(leaseId: string) {
  return query(`SELECT * FROM lease_cycles WHERE lease_id=? AND status='settled' ORDER BY period_start DESC`, [leaseId]);
}
export function getCycle(cycleId: string) { return query(`SELECT * FROM lease_cycles WHERE id=?`, [cycleId])[0]; }

function insertCycle(leaseId: string, s: Date, e: Date) {
  const id = uuidv4();
  exec(`INSERT INTO lease_cycles (id, lease_id, period_start, period_end, due_date, status)
        VALUES (?,?,?,?,?, 'open')`,
...
export function ensureCyclesForLease(leaseId: string) {
  ...
  // MONTHLY (hoặc yearly → tính như monthly nhiều tháng)
  if (lease.end_date) {
    let cursor = new Date(lease.start_date);
    while (cursor <= new Date(lease.end_date)) {
      const next = addMonths(cursor, 1);
      const e = addDays(next, -1);
      if (e > new Date(lease.end_date)) {
        insertCycle(leaseId, cursor, new Date(lease.end_date));
        break;
      } else {
    ...
    // không có end_date → tạo 1 kỳ
    const e = addDays(addMonths(s0, 1), -1);
    insertCycle(leaseId, s0, e);
  }
}
...
export function openInvoiceForCycle(cycleId: string) {
  ...
  exec(`INSERT INTO invoices (id, lease_id, period_start, period_end, issue_date, subtotal, total, status)
        ...
        FROM lease_cycles WHERE id=?`, [id, cycleId]);
...
function addInvoiceItem(
  invoiceId: string, description: string, quantity: number, unit: string|undefined,
  unit_price: number, amount: number, chargeTypeId?: string, meta?: any
) {
  const id = uuidv4();
...
function recalcInvoice(invoiceId: string) {
  const sum = query<{sum:number}>(`SELECT SUM(amount) sum FROM invoice_items WHERE invoice_id=?`, [invoiceId])[0]?.sum ?? 0;
  exec(`UPDATE invoices SET subtotal=?, total=?, status = CASE WHEN total>0 THEN status ELSE 'open' END WHERE id=?`,
    [sum, sum, invoiceId]);
}
...
export function settleCycleWithInputs(
  ...
  for (const inp of variableInputs) {
    ...
    addInvoiceItem(inv.id, ch.name, qty, ch.unit, price, qty*price, ch.charge_type_id, {
      variable:true, meter_start:Number(ch.meter_start||0), meter_end:Number(inp.meter_end||0),
      cycle_of:'current', for_period_start:c.period_start, for_period_end:c.period_end,
    });
  }
  ...
  for (const ex of extraCosts) {
    ...
    if (ex.name && amt>0) addInvoiceItem(inv.id, ex.name, 1, undefined, amt, amt, undefined, {
      extra:true, cycle_of:'current', for_period_start:c.period_start, for_period_end:c.period_end,
    });
  }
  ...
  for (const it of items) {
    if (it.charge_type_id && it.meta_json) {
      try { const m = JSON.parse(it.meta_json); if (typeof m?.meter_end === 'number') meterMap[it.charge_type_id] = m.meter_end; } catch {}
    }
  }
  for (const [ctId, endVal] of Object.entries(meterMap)) {
    const row = query<any>(`SELECT config_json FROM recurring_charges WHERE lease_id=? AND charge_type_id=? LIMIT 1`,
      [inv.lease_id, ctId])[0];
    let cfg = {};
...
export function revenueByMonth(year:number, month:number) {
  ...
  const rows = query<{sum:number}>(`SELECT SUM(total) sum FROM invoices WHERE strftime('%Y-%m', issue_date)=?`, [ym]);
  return rows[0]?.sum ?? 0;
}
...
export function hasOperatingCostSetup(apartmentId: string) {
  ...
  const c = query<{c:number}>(`SELECT COUNT(*) c FROM op_cost_templates WHERE apartment_id=?`, [apartmentId])[0]?.c ?? 0;
  return c > 0;
}

export function listOperatingCostTemplates(apartmentId: string) {
  ensureOperatingCostTables();
  return query<any>(`SELECT * FROM op_cost_templates WHERE apartment_id=? ORDER BY is_variable ASC, name ASC`, [apartmentId]);
}

export function upsertOperatingCostTemplate(apartmentId: string, name: string, isVariable: boolean, unit?: string|null, defaultAmount?: number) {
  ensureOperatingCostTables();
  const id = uuidv4();
  exec(`INSERT INTO op_cost_templates (id, apartment_id, name, is_variable, unit, default_amount)
        VALUES (?,?,?,?,?,?)`,
      [id, apartmentId, name, isVariable ? 1 : 0, unit ?? null, Number(defaultAmount||0)]);
  return id;
}

export function replaceOperatingCostTemplates(apartmentId: string, items: Array<{name:string; isVariable:boolean; unit?:string; defaultAmount?:number;}>) {
  ensureOperatingCostTables();
  exec(`DELETE FROM op_cost_templates WHERE apartment_id=?`, [apartmentId]);
  for (const it of items) {
    upsertOperatingCostTemplate(apartmentId, it.name.trim(), !!it.isVariable, it.unit ?? null, Number(it.defaultAmount||0));
  }
}

export function listOperatingCostMonths(apartmentId: string) {
  ensureOperatingCostTables();
  return query<any>(`SELECT * FROM op_cost_months WHERE apartment_id=? ORDER BY ym DESC`, [apartmentId]);
}

export function ensureOperatingCostMonth(apartmentId: string, ym: string) {
  ensureOperatingCostTables();
  // month row
  let m = query<any>(`SELECT * FROM op_cost_months WHERE apartment_id=? AND ym=? LIMIT 1`, [apartmentId, ym])[0];
  if (!m) {
    const mid = uuidv4();
    ...
    for (const t of tpls) {
      const iid = uuidv4();
      exec(`INSERT INTO op_cost_items (id, month_id, name, is_variable, unit, amount)
            VALUES (?,?,?,?,?,?)`,
...
export function getOperatingMonth(apartmentId: string, ym: string) {
  ensureOperatingCostTables();
  const m = ensureOperatingCostMonth(apartmentId, ym);
  const items = query<any>(`SELECT * FROM op_cost_items WHERE month_id=? ORDER BY is_variable ASC, name ASC`, [m.id]);
  return {month: m, items};
}

export function saveOperatingMonth(apartmentId: string, ym: string, items: Array<{id?:string; name:string; is_variable:number; unit?:string|null; amount:number;}>) {
  ensureOperatingCostTables();
  const m = ensureOperatingCostMonth(apartmentId, ym);
...
export function closeExpiredLeases() {
  ...
  const expired = query<any>(`
    ...
    WHERE l.status='active' AND l.end_date IS NOT NULL AND l.end_date < ?`, [today]);
...
export function ensureLeaseCollectColumn() {
  try { exec(`ALTER TABLE leases ADD COLUMN base_rent_collect TEXT DEFAULT 'start'`); } catch {}
}
...
export function isLastCycle(cycleId: string) {
  ...
  if (lease.end_date) { // chu kỳ cuối nếu period_end >= end_date
    return c.period_end >= lease.end_date;
  }
  const more = query<{cnt:number}>(`
    SELECT COUNT(*) cnt
    FROM lease_cycles
    WHERE lease_id = ? AND period_start > ?
  `, [lease.id, c.period_end])[0]?.cnt ?? 0;
...
// ====== Operating costs (by apartment/month) ======
...
export function getApartmentStats() {
  ...
  if (!apt) return null;
  const total = query<{c:number}>(`SELECT COUNT(*) c FROM rooms WHERE apartment_id=?`, [apt.id])[0]?.c ?? 0;
  const occupied = query<{c:number}>(`SELECT COUNT(*) c FROM rooms WHERE apartment_id=? AND status='occupied'`, [apt.id])[0]?.c ?? 0;
...
export function listOperatingMonths(apartmentId: string): string[] {
  ...
  if (startRow?.created_at) {
    ...
    const anyRoom = query<any>(`SELECT created_at FROM rooms WHERE apartment_id=? ORDER BY created_at ASC LIMIT 1`, [apartmentId])[0];
...
export function ensureOperatingMonth(apartmentId: string, ym: string) {
  ensureOperatingTables();
  const exist = query<{c:number}>(`SELECT COUNT(*) c FROM operating_months WHERE apartment_id=? AND ym=?`,
    [apartmentId, ym])[0]?.c ?? 0;
  if (exist>0) return;
  const id = uuidv4();
  exec(`INSERT INTO operating_months (id, apartment_id, ym) VALUES (?,?,?)`, [id, apartmentId, ym]);

  // auto add fixed templates
  const tpls = query<any>(`SELECT * FROM operating_fixed_templates WHERE apartment_id=?`, [apartmentId]);
  for (const t of tpls) {
    const eid = uuidv4();
    exec(`INSERT INTO operating_expenses (id, apartment_id, ym, name, amount, type)
          VALUES (?,?,?,?,?, 'fixed')`, [eid, apartmentId, ym, t.name, t.amount]);
  }
}

export function listOperatingExpenses(apartmentId: string, ym: string) {
  ensureOperatingTables();
  return query(`SELECT * FROM operating_expenses WHERE apartment_id=? AND ym=? ORDER BY rowid ASC`, [apartmentId, ym]);
}

export function addOperatingExpense(apartmentId: string, ym: string, exp: OperatingExpense) {
  ensureOperatingMonth(apartmentId, ym);
  const id = uuidv4();
  exec(`INSERT INTO operating_expenses (id, apartment_id, ym, name, amount, type, note)
        VALUES (?,?,?,?,?,?,?)`,
...
export function listFixedExpenseTemplates(apartmentId: string) {
  ensureOperatingTables();
  return query(`SELECT * FROM operating_fixed_templates WHERE apartment_id=? ORDER BY rowid ASC`, [apartmentId]);
}
export function addFixedExpenseTemplate(apartmentId: string, tpl: {name:string; amount:number}) {
  ensureOperatingTables();
  const id = uuidv4();
  exec(`INSERT INTO operating_fixed_templates (id, apartment_id, name, amount) VALUES (?,?,?,?)`,
...
export function getReportOperatingExpenses(apartmentId:string, start:string, end:string) {
  ...
  `, [apartmentId, start, end]).map((x:any)=>({...x, amount: Number(x.amount)||0}));
...
export function upsertRecurringChargeForLease(
  ...
  return addRecurringCharge(leaseId, ctId, item.price, item.isVariable ? 1 : 0, item.isVariable ? { meter_start: item.meterStart || 0 } : undefined);
}
...
export function revenueAndExpenseByApartmentForMonth(year: number, month: number) {
  ...
  const totals = rows.reduce(
    (s, r) => ({ revenue: s.revenue + r.revenue, expense: s.expense + r.expense, profit: s.profit + r.profit }),
