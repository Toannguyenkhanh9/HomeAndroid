...
export async function exportReceiptPdf(invoiceId: string) {
  ...
  const html = `<html><head><meta charset="utf-8" /></head><body style="font-family:-apple-system,Roboto,Arial;padding:16px"><h2>Biên nhận thanh toán</h2><div>Cho kỳ thuê: <b>${inv.period_start}</b> → <b>${inv.period_end}</b></div><div>Ngày: ${new Date().toISOString().slice(0,10)}</div><div>Đã nhận: <b>${inv.total.toLocaleString('vi-VN')} đ</b></div></body></html>`;
