...
import {
  ...
  parseDecimalSafe,
  parseDecimalComma,
  formatDecimalTypingComma,
  formatDecimalTypingCommaSmart,
  parseDecimalCommaSmart,
...
export default function LeaseForm({ route, navigation }: Props) {
  ...
  const addCharge = () =>
    setCharges(p => [
      ...p,
      { name: '', isVariable: false, unit: 'tháng', price: '' },
    ]);
  ...
  function submit() {
    ...
    const baseRent = allInclusive
      ? parseAmount(allInclusiveAmount)
      : parseAmount(baseRentText);
    ...
    >['charges'] = !allInclusive
      ? charges
          ...
          .map(ch => ({
            ...
            meterStart: ch.isVariable
              ? parseIntSafe(ch.meterStart || '')
              : undefined,
          }))
      : undefined;
    ...
    const durationDays =
      term === 'fixed' && mode === 'daily' ? parseAmount(days) || 1 : undefined;
    ...
    try {
      ...
      Alert.alert(title, msg, [
        {
          text: t('common.ok') || 'OK',
          onPress: () => navigation.replace('LeaseDetail', { leaseId }),
        },
      ]);
    } catch (e: any) {
      ...
      <ScrollView contentContainerStyle={{ padding: 12, gap: 12 }}>
        ...
        <Card style={{ gap: 10 }}>
          ...
          >
            <Text style={{ color: c.text }}>
              {formatDateISO(startISO, dateFormat, language)}
            </Text>
          </TouchableOpacity>
          {showDatePicker && (
            <DateTimePicker
        ...
        <Card style={{ gap: 8 }}>
          ...
          {allInclusive ? (
            <>
              <Text style={{ color: c.subtext }}>
                {t('leaseForm.allInclusiveAmountHint')}
              </Text>
              <TextInput
                keyboardType="numeric"
                value={allInclusiveAmount}
                onChangeText={ttext =>
                  setAllInclusiveAmount(formatTyping(ttext))
                }
            ...
            {charges.map((ch, idx) => (
              <View key={idx} style={{ borderRadius: 10, padding: 10, gap: 8 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  ...
                  >
                    <Text style={{ color: ch.isVariable ? c.text : 'white' }}>
                      {t('leaseForm.fixed')}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={() =>
                      updCharge(idx, { isVariable: true, unit: t('rent.unit') })
                    }
                    style={{
                      paddingHorizontal: 12,
                      paddingVertical: 6,
                  ...
                  >
                    <Text style={{ color: ch.isVariable ? 'white' : c.text }}>
                      {t('leaseForm.variable')}
                    </Text>
                  </TouchableOpacity>
                </View>
                ...
                <Text style={{ color: c.subtext }}>
                  {ch.isVariable
                    ? t('leaseForm.unitPricePerUnit')
                    : t('leaseForm.unitPricePerPeriod')}
                </Text>
                <TextInput
