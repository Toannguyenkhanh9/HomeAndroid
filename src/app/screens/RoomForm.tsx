...
export default function RoomForm({
  ...
  const filtered = React.useMemo(() => {
    ...
    return rooms.filter(
      r =>
        r.code.toLowerCase().includes(ttxt) ||
        r.status.toLowerCase().includes(ttxt),
    );
  }, [rooms, q]);
    ...
    >
      <FlatList
              ...
              {apt ? (
                <Card>
                  ...
                  <Text style={{ color: c.text, marginTop: 6 }}>
                    {t('roomForm.total')}: {apt.total} •{' '}
                    {t('roomForm.occupied')}: {apt.occupied} •{' '}
                    {t('roomForm.available')}: {apt.available}
                  </Text>
                  ...
                  >
                    <Button
                      ...
                      onPress={() =>
                        navigation.navigate('OperatingCosts', { apartmentId })
                      }
                    />
                    <Button
                      title={t('catalog.title') || 'Bảng giá'}
                      onPress={() =>
                        navigation.navigate('ChargeCatalog', { apartmentId })
                      }
                    />
                    <Button
                      title={t('roomForm.report')}
                      onPress={() =>
                        navigation.navigate('ApartmentReport', { apartmentId })
                      }
                    />
                  </View>
                </Card>
        ...
        ListEmptyComponent={
          <View>
            <Card>
              <Text style={{ color: c.subtext }}>{t('roomForm.noRooms')}</Text>
            </Card>
          </View>
        }
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onLongPress={() => {
              Alert.alert(
                t('roomForm.deleteRoom'),
                t('roomForm.confirmDelete', { code: item.code }),
                [
